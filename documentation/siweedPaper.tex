...
\[\tau = kp * p + -kd * v\]
Where $\tau$ is the calculated torque, $kp$ is the user defined proportional gain, $p$ is the position in meters, $kd$ is the user defined derivative gain, and $v$ is the velocity, in meters per second, caclulated by the change in position over the last 10 milliseconds. 
...
\figurename~\ref{fig:XX} shows the controller response with zero D gain, and stepped P gain. \figurename~\ref{fig:XX} Is similar, but with zero P gain and stepped D gain.
