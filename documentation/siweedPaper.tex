...
Delaney Heileman , 
A. Gerrit Motes \orcidB{}, 
Anwi Fomukong , 
Giorgio Bacelli \orcidC{}, \linebreak  
Steven J. Spencer \orcidE{}, 
Dominic D. Forbush \orcidD{}, 
Kevin Dullea  and 
Ryan G. Coe *\orcidA{}}
...
\AuthorCitation{Ross, N.; Heileman, D.; Motes, A.G.; Fomukong, A.; Bacelli, G.; Spencer, S.J.; Forbush, D.D.; Dullea, K.; Coe, R.G.}
...
\abstract{
Wave energy converters are a nascent energy generation technology that harnesses the power in ocean waves.
...
This paper presents the detailed system design of this tool.
Furthermore, via testing and analysis, we demonstrate the basic functionality of the system.
}
...
The Sandia Interactive Wave Energy Education Display (SIWEED; a video showing the system in operation is available online: %MDPI: 1. Footnotes are not supported in our journal. We have therefore included this paragraph in the main text. Please confirm. 2. Please add the access date for ALL URLs (Format: Date Month Year). e.g., (accessed on 1 January 2020). It should be before the accepted date. The following highlights are the same. %RGC: ok
...
\begin{itemize}
  \item \textbf{Open-source and documented design:} %MDPI: Please confirm if the bold formatting is necessary; if not, please remove it. The following highlights are the same. % RGC: prefer to keep bold formatting
...
%MDPI: Please confirm the alignment change. RGC: confirmed
%MDPI: Please note: 1. Figures and Tables should be cited after where they are first mentioned, we have adjusted all. 2. The size and position of figures/tables will be adjusted appropriately to avoid blank space and to ensure the clarity and readability of the image during the final steps. RGC: OK
...
\begin{figure}[H]
  ...
  \caption{Graphical user interface (GUI) screenshot.}
   %MDPI: Please change the hyphen (-) into a minus sign (−) in the figure, e.g., “-1” should be “−1”. Same for below Figures 5-7.
   % RGC: we cannot make this change, this is a bitmap, not a vector graphic and the graphics were generated from the Processing library, so decision about characters used in plots were made by Processing
...
\end{equation}
%
Here, %RGC: no, this should not be indented as a new paragraph
...
%\DTLloaddb{data}{BOM_no_decorations.csv}
%
%\begin{landscape}
%\begingroup
%\footnotesize
...
%
%\DTLdisplaylongdb[caption=Bill of materials (note: decorations for town not included)., contcaption=Bill of materials (continued)., label=tab:bom]{data}
%\end{landscape}
%\endgroup
...
%MDPI: 1. We moved the table code from csv file to tex. Please double-check if they are correct. 2. To avoild blank space, we moved Table 2 here. Please confirm.
% RGC: Confirmed.
\label{tab:bom}
\begin{tabularx}{\textwidth}{llLrrrl}
\toprule
\textbf{Group} & \textbf{Designator} & \textbf{Component} %MDPI: The companies/manufacturers of chemicals & reagents, devices, instruments, commercial cell lines/samples/materials should be indicated together with their city (states abbreviation is required for USA and Canada) and country in their first appearance. Please review the full text carefully and add the necessary information. Example: Analyzer (Applied Biosystems, Waltham, MA, USA). RGC: confirmed
 & \textbf{Number} & \textbf{Cost per Unit {[}USD{]}} %English Editor: Please check that intended meaning has been retained. RGC: confirmed
...
On the~laptop:
\begin{enumerate}
\item Download the latest version of Arduino IDE and Processing IDE. %MDPI: Please state the version number of the software. If it is not applicable, the related website or proof should be provided. Please review the full text and make sure each software is mentioned in main text and has a version number or website. % RGC: we already state "latest version"
...
\begin{figure}[H]
  ...
  \caption{Torque (estimated based on hydrostatics) vs. commanded current. Linear trends are fit to estimate/verify motor torque~constant.} % RGC: this figure is the standard output from MATLAB, so I will not be changing it per your suggestion of "hyphen (-) into a minus sign (−)"
...
\begin{figure}[H]
  ...
  \caption{Motor torque error compared with commanded motor~current.} % RGC: this figure is the standard output from MATLAB, so I will not be changing it per your suggestion of "hyphen (-) into a minus sign (−)"
...
\begin{figure}[H]
  ...
  \caption{Comparison %MDPI: Please use scientific notations in the figure, e.g., “8 × 10³”, not “8E3”. % RGC: Updated
...
%MDPI: Please check if any short description about Supplementary Materials should be added.
\vspace{12pt}
{\footnotesize
\setlength{\cellWidtha}{\columnwidth/3-2\tabcolsep--0.3in}
\setlength{\cellWidthb}{\columnwidth/3-2\tabcolsep-0.9in}
\setlength{\cellWidthc}{\columnwidth/3-2\tabcolsep--0.6in}
\noindent
\scalebox{1}[1]{\begin{tabularx}{\columnwidth}{>{\PreserveBackslash\raggedright}m{\cellWidtha}>{\PreserveBackslash\raggedright}m{\cellWidthb}>{\PreserveBackslash\raggedright}m{\cellWidthc}}
\toprule

\textbf{\boldmath{File Name}} & \textbf{\boldmath{File Type}} & \textbf{\boldmath{Description}}\\
\cmidrule{1-3}
...
documentation 					& Directory & Journal paper; electrical~diagrams\\
...
\funding{This project was supported by %MDPI: Please check the accuracy of funding data (especially funding number) and any other information carefully. Any errors may affect your future funding. % RGC: confirmed
...
\acknowledgments{Francisco Colorbio, John Quinlan, and~Sean Pluemer worked on an earlier version of the SIWEED project on which this current version was based.
...
% \appendixstart
% \appendix
% \section[\appendixname~\thesection]{Bill of materials\label{sec:detailed_design_documents}}
