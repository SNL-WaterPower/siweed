...
\hline \textbf{Cost of hardware} &
  ...
  \textit{\$6763 - Approximate cost of hardware (complete breakdown will be included in the Bill of Materials).}
  \\
...
There seem to be two major iterations, one electrically driven by a paddle like wavemaker, and one plunger type driven mechanically by hand. 
\citet{Trust2015} has also make multiple similar hydraulic flume tanks for demonstration purposes.
...
This is a novel expansion on similar small-scale educational wave tanks, as the user is able to control both the waves and the WEC device, and the Wavemaker is driven by a ballscrew.
...
The ``System Status" section shows two plots, one for the WEC and one for the Wavemaker, with each allowing the user to choose what data is plotted. Below the plots are an FFT of the generated waves, and a radial meter that displays the hypothetical generated power of the WEC.

Two Arduino Due microcontrollers are used to control and acquire signals from the WEC and Wavemaker.
...
\item File $>$ Open $>$ siweedGUI.pde (Ex: C\textbackslash Users\textbackslash user\textbackslash Documents\textbackslash GitHub\textbackslash siweed\textbackslash Processing\textbackslash siWeedGUI\textbackslash siweedGUI.pde) 
...
Demonstrate the operation of the hardware and characterize its performance over relevant critical metrics -XX
\begin{itemize}
\item \textbf{Torque constant verification:} 
The factory torque constant was verfied by logging the position and current of the WEC while commanding a slow sinusoidal current.
...
Plotting this torque data against the recorded commanded current values gives the following(only one of the three data sets used is shown here):
-XX add vector graphics
The data points at the highest and lowest currents are affected by static friction at the extreme values, so cutting them from the data set improves the torque constant estimation:
-XX
This gives us an estimated torque constant of about 7.52mNm/A, which is within 4\% of the factory stated constant of 7.8mNm/A
\linebreak
...
This allows the friction to be seen as half the offset between the data points as the wavemaker moves up, and the points as it moves down. 
Plotting the torque error using the calculated torque constant against the current gives the following:
-XX
The average offset shows that the friction is about 0.45mNm.

\item \textbf{Wave Probes:}The readings from the wave probes could be verified by attaching them to the wave maker, which has a very precise encoder that allows for accurate changes in displacement. 
