...
class FFTbase {
   ...
   */
  public float[] fft(float[] inputReal, float[] inputImag, boolean DIRECT) {
...
void make_signal() {
  ...
  for (int i=0; i<many; i++) signalr[i] = off_ + 0.5*cos(ang*i)+ 0.1*cos(owelle*ang*i)+0.7*cos(2*owelle*ang*i);
}
